...
vim.keymap.set('n', '<leader>o', function() fzf_lua.files() end, { noremap = true })
vim.keymap.set('n', '<leader>O', function()
    fzf_lua.files({
        cmd = "fd --type f --hidden --exclude .git --color=always"
    })
end, { noremap = true })
vim.keymap.set('n', '<leader>f', function() fzf_lua.live_grep() end, { noremap = true })
...
vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function()
        ...
        vim.keymap.set('n', '<leader>f', function()
            fzf_lua.live_grep({
                rg_opts =
                "--column --line-number --hidden --no-heading --color=always --smart-case -g '!.git/' -g '!build/' -g '!bin/'",
            })
        end, { noremap = true, buffer = true })
    end,
})
...
vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function()
        ...
        vim.keymap.set('n', '<leader>f', function()
            fzf_lua.live_grep({
                rg_opts =
                "--column --line-number --hidden --no-heading --color=always --smart-case -g '!.git/' -g '!target/' -g '!dist/'",
            })
        end, { noremap = true, buffer = true })
    end,
})
